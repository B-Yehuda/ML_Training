...
def get_tables_and_views(conn: Connection,
                         schema: str = None
                         ) -> pd.DataFrame:
    ...
    # execute query
    df_tables_and_views = pd.read_sql(sql, conn)

    return df_tables_and_views


def create_empty_table(cur: Connection,
                       schema: str,
                       table_name: str,
                       columns_and_types_dict: dict
                       ):
    # define column name and data type
    lines = []
    ...
    print(f"- Created empty table: \033[1m{schema}.{table_name}\033[0m")
...
def drop_table_if_exists(cur: Connection,
    ...
    print(f"- Dropped existing table: \033[1m{schema}.{table_name}\033[0m")
...
def actual_create_table(engine: Connection,
    ...
    if skip_if_exists and table_exists(cur, table_name, schema):
        print(f"Table {schema}.{table_name} already exists, skipping.")
        return

    # drop table if exists
    if drop_if_exists and table_exists(cur, table_name, schema):
        drop_table_if_exists(cur=cur, schema=schema, table_name=table_name)

    print(f"Creating {schema}.{table_name} - started at: \033[1m{datetime.now().isoformat(' ', 'seconds')}\033[0m")
    ...
    print(f"Creating {schema}.{table_name} - finished at: \033[1m{datetime.now().isoformat(' ', 'seconds')}\033[0m")
...
def table_exists(conn: Connection,
    ...
    df_tables = get_tables_and_views(conn=conn, schema=schema)
